...
| --- | --- | --- | --- |
| `Normal_asphalt` | An intact, paved asphalt road surface with no visible target condition. | Clean asphalt, harmless texture variation, road markings, manholes, shadows, stains, or puddles when there is no visible damage. | Any visible pothole, crack, repaired patch, or primarily unpaved surface. |
...
4. Never label a shadow, road marking, manhole, water puddle, oil stain, or dark patch as a pothole without a visible hole or depression.
...
- A clear intact asphalt road with shadows: `Normal_asphalt`.
...
`Normal_asphalt` is derived only when all four labels are `No` and the image is not excluded. An image may therefore be both `pothole_present=Yes` and `crack_present=Yes`.
