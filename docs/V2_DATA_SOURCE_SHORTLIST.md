...
| --- | --- | --- | --- | --- |
| [RDD2022 / RoadDamageDetector](https://github.com/sekilab/RoadDamageDetector) | Official project documentation states that the images are CC BY-SA 4.0. RDD2022 contains multinational road imagery with four road-damage labels: longitudinal crack, transverse crack, alligator crack, and pothole. | `Pothole`, `Crack` | Object detection; multi-label after a documented label conversion | **Primary candidate** for real damage examples. It does not provide the full V2 look-alike label set. |
...
Select one primary candidate and inspect its dataset card or download page manually for the exact licence, terms, available files, annotation format, and size. Only then create a reproducible download/access instruction and perform the V2 data audit.
