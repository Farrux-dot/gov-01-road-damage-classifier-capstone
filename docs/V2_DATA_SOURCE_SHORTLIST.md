...
| --- | --- | --- | --- | --- |
| [SVRDD_YOLO](https://huggingface.co/datasets/ShuoZheLi/SVRDD_YOLO) | Dataset card lists 8,000 images, 20,804 annotated objects, a total size of 3.92 GB, and YOLO-format annotations. Its classes include three crack types, potholes, manhole covers, and longitudinal/transverse patches. The card states CC BY 4.0. | `Pothole`, `Crack`, `Manhole`, `Repaired_road` | Object detection; multi-label after conversion | **Acquired on 2026-09-02; audit pending.** The three image archives and their matching annotation metadata have been checked for count and filename agreement. |
...
Run the SVRDD data audit. The three source-provided image archives and their matching metadata are stored under the ignored `data/raw/v2/svrdd/` location. Do not use the supplied test split as V2's final protected test without duplicate and source-overlap checks.
