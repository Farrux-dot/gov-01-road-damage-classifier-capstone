...
- The ten-class multi-class, nine-output multi-label, and nine-class detection targets remain blocked. Accepted dedicated examples are still missing for shadow, puddle, road marking, and road stain. StreetSurfaceVis now supplies candidate full-image coverage for normal asphalt and unpaved road only.
...
- Next small task: review source URLs and licences, then preflight one traceable full road-scene source for the remaining `Shadow`, `Puddle`, `Road_marking`, and `Road_stain` classes. Do not download a source until its visible examples are approved.
