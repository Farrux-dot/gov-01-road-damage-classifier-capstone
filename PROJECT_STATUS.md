...
- PaveBench, RAD, Deep Pavements, and CoCGRCDD were retired from the current V2 work and are not inputs to the candidate inventory.
- The source-traceable pre-split candidate inventory was rebuilt on 2026-09-19. It contains 14,103 records: 6,000 SVRDD source-training records, 624 eligible V1 training potholes, 1,241 GitHub pothole records, 1,721 eligible StreetSurfaceVis source-training records, 2,097 duplicate-safe CeyMo source-training records, 1,197 Hugging Face manhole-cover records, 1,076 audited Kaggle source-training speed-bump records, and 147 Mendeley speed-breaker records. See `docs/V2_CANDIDATE_INVENTORY.md` and `docs/v2_candidate_inventory.csv`.
- The new GitHub source contributes 4,061 valid YOLO pothole boxes. Its two exact duplicate copies and one zero-width source box line are excluded by `docs/v2_github_pothole_candidate_manifest.csv`; it has no official split and remains a pre-split candidate source.
- The inventory keeps all V1 validation/protected-test images, all SVRDD source validation/test images, all StreetSurfaceVis records marked `official_train=False`, the CeyMo official test package, and all 1,277 Kaggle speed-bump source-test images out of the candidate pool. It found zero exact SHA-256 duplicate groups across the 14,103 included records. Three conflicting Mendeley Good_Manhole/Speed_Breaker image pairs remain held out.
- Of the candidates, 9,338 have object boxes; 4,765 image-level candidates have no boxes and cannot support object detection without new box annotations. This includes V1 potholes, StreetSurfaceVis surfaces, Hugging Face manhole covers, Kaggle speed bumps, and Mendeley speed breakers.
...
- The ten-class multi-class, nine-output multi-label, and nine-class detection targets remain blocked. Accepted dedicated examples are still missing for shadow, puddle, and road stain. StreetSurfaceVis supplies candidate full-image coverage for normal asphalt and unpaved road, while CeyMo now supplies audited road-marking candidates that still need inventory integration.
...
- StreetSurfaceVis was audited as a source for `Normal_asphalt` and `Unpaved_road`: the 9,122-image archive contains 971 normal-asphalt candidates and 1,016 unpaved-road candidates.
- The resolution rule was corrected on 2026-09-13. Images are no longer excluded only because their longest side is below 1,024 pixels. Both dimensions must be at least 224 pixels. This adds 42 source-training candidates to each target class, producing 791 normal-asphalt and 931 unpaved-road resolution-eligible, exact-deduplicated training candidates.
- The 17 below-1,024 candidates marked `official_train=False` remain protected and are not added to training. The completed 100-image review records 40/40 normal-asphalt keeps, 59/60 unpaved-road keeps, and one unresolved unpaved sample (`SSV-055`). These are review findings, not model metrics.
- StreetSurfaceVis is marked as multi-class-only evidence because it has no object boxes and does not supply complete nine-output multi-label truth. Exact SHA-256 checking across those 8,454 records found zero duplicate groups. No final V2 split was created and no V2 model was trained.
- CeyMo's official 2,099-image training set was downloaded and audited on 2026-09-13 after the student approved the visible image quality and road-marking examples. All images and masks are readable at 1,920 x 1,080, and the XML and polygon records agree on 3,488 road-marking objects.
- The CeyMo structural audit passed with warnings: two exact duplicate image pairs, one subpixel polygon boundary-rounding case, one incorrect JSON `imagePath`, and a `BL`/`CL` count discrepancy relative to the official paper. These warnings and the exact 11-label mapping are documented in `docs/V2_CEYMO_DATA_AUDIT.md` and `docs/V2_CEYMO_LABEL_MAPPING.md`.
- CeyMo was integrated into the pre-split inventory on 2026-09-13. The dedicated manifest records all 2,099 source images, excludes `602b.jpg` and `618c.jpg` as redundant copies, and retains 2,097 images with 3,484 road-marking boxes. The duplicate with more objects was kept; equal object counts used source ID order as a stable tiebreak.
- CeyMo supports `road_marking_present=Yes` and object detection, but it does not automatically provide a multi-class primary label or prove that other V2 conditions are absent. No final V2 split has been created and no V2 model has been trained.
- Next small task: preflight traceable sources for the remaining `Shadow`, `Puddle`, and `Road_stain` gaps.
