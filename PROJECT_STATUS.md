...
- The source-traceable pre-split candidate inventory was created on 2026-09-09 and expanded with StreetSurfaceVis on 2026-09-13. It contains 8,454 records: 6,000 SVRDD source-training records, 624 eligible V1 training potholes, 109 individually approved PaveBench detection records, and 1,721 eligible StreetSurfaceVis source-training records. See `docs/V2_CANDIDATE_INVENTORY.md` and `docs/v2_candidate_inventory.csv`.
- The inventory keeps all V1 validation/protected-test images, all SVRDD source validation/test images, and all StreetSurfaceVis records marked `official_train=False` out of the candidate pool. It found zero exact SHA-256 duplicate groups across the 8,454 included records.
- Of the candidates, 6,109 have object boxes; the 624 V1 pothole images and 1,721 StreetSurfaceVis images have image-level labels only and cannot support object detection without new box annotations.
...
- StreetSurfaceVis was integrated into the source-traceable pre-split inventory on 2026-09-13. The unclear record `SSV-055` and every `official_train=False` record were excluded. The inventory now contains 8,454 records: 6,000 SVRDD, 624 V1 training potholes, 109 individually approved PaveBench records, and 1,721 StreetSurfaceVis records (791 normal asphalt and 930 unpaved road).
- StreetSurfaceVis is marked as multi-class-only evidence because it has no object boxes and does not supply complete nine-output multi-label truth. Exact SHA-256 checking across all 8,454 inventory records found zero duplicate groups. No final V2 split was created and no V2 model was trained.
...
- CeyMo is accepted as a candidate source for `Road_marking`, but it has not yet been deduplicated or integrated into `docs/v2_candidate_inventory.csv`. It does not prove that other V2 conditions are absent from its images.
- Next small task: build a source-traceable, duplicate-safe CeyMo candidate manifest and integrate it into the pre-split inventory. After that, continue source preflight for the remaining `Shadow`, `Puddle`, and `Road_stain` gaps.
